$(function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                  ...
                  sessionId =  data.value.userId

                  //查询订单
                  order_query(tokenId, sessionId)
            }
        })
        ...
        $.ajax({
            ...
            dateType: 'json',
            success: function(data) {
                  sessionId = data.value.user_id

            }
        })
})

function order_query(tokenId, sessionId) {
    ...
    $('.query').click(function () {
        ...
        $.ajax({
            url: 'http://demo.jomton.com:3000/business/query',
            data: {
                start_day: start_day,
                end_day: end_day,
                tokenId: sessionId,
            ...
            success: function(data) {
                ...
                for (var j = 0 , i = j + 1; j < Info.count; j++ , i++) {
                    ...
                    $('.orders_table').append("<tr><td class='order_num'>"
                                            + i + "</td><td>" + Info.rows[j].bigMatch.name
